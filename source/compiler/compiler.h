...
COMP__lexlings COMP__compile__lex(ANVIL__buffer user_code, COMP__file_index file_index, COMP__error* error) {
    ...
    while (COMP__check__current_within_range(current)) {
        ...
        if (COMP__calculate__valid_character_range(current, '(', '(')) {
            ...
            // append lexling
            COMP__append__lexling(&output, COMP__create__lexling(COMP__lt__string_literal, data, COMP__create__character_location(file_index, current_line_number, COMP__calculate__character_index(user_code, current))), error);
...
COMP__accountling_argument COMP__create__accountling_argument(COMP__pat type, COMP__accountling_index index) {
...
COMP__accountling_argument COMP__create_null__accountling_argument() {
    return COMP__create__accountling_argument(COMP__pat__invalid, COMP__define__null_call_ID);
}
...
COMP__accountling_statement COMP__create__accountling_statement(COMP__accountling_abstraction_header header, ANVIL__list inputs, ANVIL__list outputs) {
...
COMP__accountling_statement COMP__create_null__accountling_statement() {
    return COMP__create__accountling_statement(COMP__create_null__accountling_abstraction_header(), ANVIL__create_null__list(), ANVIL__create_null__list());
}

// open statement
COMP__accountling_statement COMP__open__accountling_statement(COMP__accountling_abstraction_header header, COMP__error* error) {
    COMP__accountling_statement output;
    ...
    // init lists
    output.inputs = COMP__open__list(sizeof(COMP__accountling_argument) * 8, error);
    if (COMP__check__error_occured(error)) {
        return output;
    }
    output.outputs = COMP__open__list(sizeof(COMP__accountling_argument) * 8, error);
    if (COMP__check__error_occured(error)) {
        return output;
    }
...
ANVIL__list COMP__generate__call_blueprint(ANVIL__list parsling_programs, COMP__error* error) {
    ...
    while (COMP__check__current_within_range(current_blueprintling) && (*(COMP__blueprintling*)current_blueprintling.start) == COMP__abt__define_call) {
        ...
        // set name
        header.header.name = COMP__create__parsling_argument(COMP__pat__variable, COMP__create__name(COMP__create__lexling(COMP__lt__name, ANVIL__open__buffer_from_string((ANVIL__u8*)names[*(COMP__blueprintling*)current_blueprintling.start], ANVIL__bt__false, ANVIL__bt__false), COMP__create_null__character_location())), 0);
        current_blueprintling.start += sizeof(COMP__blueprintling);
...
COMP__accountling_program COMP__create__accountling_program(ANVIL__list call_blueprint, ANVIL__list abstractions, ANVIL__list predefined_variables, COMP__call_index main_abstraction_ID) {
...
COMP__accountling_program COMP__create_null__accountling_program() {
    return COMP__create__accountling_program(ANVIL__create_null__list(), ANVIL__create_null__list(), ANVIL__create_null__list(), COMP__define__null_call_ID);
}
...
ANVIL__list COMP__account__accountling_argument_list(COMP__accountling_abstraction* abstraction, ANVIL__list parsling_arguments, COMP__error* error) {
    ...
    while (COMP__check__current_within_range(current_argument)) {
        ...
        if (argument.type == COMP__pat__variable__input) {
            COMP__append__accountling_argument(&output, COMP__create__accountling_argument(argument.type, COMP__find__parsling_argument_index__by_name((*abstraction).inputs, argument)), error);
...
COMP__accountling_program COMP__account__program(ANVIL__list parsling_programs, COMP__error* error) {
    ...
    // search for main
    // setup current
    COMP__current current_header = COMP__calculate__current_from_list_filled_index(&output.abstractions);
...
typedef struct COMP__generation_offsets {
    ...
    ANVIL__list statement_offsets; // the function specific statement offsets
...
// open workspace
COMP__generation_workspace COMP__open__generation_workspace(ANVIL__buffer* program_buffer, COMP__accountling_program accountlings, COMP__error* error) {
...
// generate function
void COMP__forge__anvil_abstraction(COMP__generation_workspace* workspace, COMP__abstraction_index generation_abstraction_index, COMP__accountling_abstraction accountling_abstraction, COMP__error* error) {
    COMP__generation_abstraction* generation_abstraction;

    // setup generation abstraction address
    generation_abstraction = (COMP__generation_abstraction*)((*workspace).abstractions.buffer.start + (sizeof(COMP__generation_abstraction) * generation_abstraction_index));
...
void COMP__forge__anvil_program(ANVIL__buffer* final_program, COMP__accountling_program accountlings, ANVIL__stack_size stack_size, COMP__error* error) {
    ...
    COMP__generation_workspace workspace = COMP__open__generation_workspace(final_program, accountlings, error);
    if (COMP__check__error_occured(error)) {
        return;
    }
    ...
    for (ANVIL__pt pass = ANVIL__pt__get_offsets; pass < ANVIL__pt__COUNT; pass++) {
        // setup pass
        ANVIL__setup__pass(&workspace.anvil, pass);
